...
> 專案**沒有單元測試框架**（composer.json 裡沒有 phpunit，phpcs.xml 也把 `tests/*` 排除）。需要跑測試請先確認使用情境，不要假設測試已經設好。
...
5. **Allowed domains 白名單**：`get_allowed_domains()` 會打 `https://cloud.luke.cafe/wp-json/power-partner-server/allow_domains` 取得免 LC 的網域清單，cache 14 天；失敗時 fallback 到 hard-coded 清單並**照樣 cache**（避免重複打爆 API，見最近 commit `a9aa865`）。
...
7. **i18n**：`load_textdomain()` 用 **kebab-case**（不是 snake！）作為 textdomain，路徑為 plugin 根目錄下的 `languages/`。見 [src/traits/PluginTrait.php:642-655](src/traits/PluginTrait.php#L642-L655) 與 commit `db355ff` — 這是為了跟 plugin 主檔 `Text Domain:` header 對齊，**不要改回 snake，會讓所有 `__()` 呼叫靜默失效**。
...
`\J7\Powerhouse\*` 是 soft dependency — 程式碼用 `class_exists()` 判斷。**修改時不要假設它一定存在**。

## 寫新 trait / 改既有 trait 的注意事項
...
3. 使用 `self::$var` 時要記得這是 **trait 裡的 late static binding**，使用方 class 會共享 trait 內宣告的 `public static` 屬性 — 多個 plugin 各自 `use` 時會有各自的 static state，但同一個 class 不同位置呼叫都打到同一組 static。
4. 對 WordPress 全域函式使用**加 backslash 前綴**（如 `\add_action`、`\get_option`），這是既有風格，phpstan stubs 會接住。
5. 版本在 [composer.json:3](composer.json#L3)；發 release 時要同步更新 — 此 library 自身也是透過 plugin-update-checker 派送給下游 plugin 的。
